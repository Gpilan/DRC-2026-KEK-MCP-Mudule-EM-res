...
int main(int argc, char** argv) {
    ...
    std::string full_ch_name = "";
    for (int idx = 0; idx < channel_names.size(); idx++) {
        full_ch_name += channel_names.at(idx) + "_";
    }
    full_ch_name = full_ch_name.substr(0, full_ch_name.size() - 1);

    fs::path dir3("./Waveform/Run_" + std::to_string(fRunNum) + "/" + full_ch_name);   
    if (!(fs::exists(dir3))) fs::create_directory(dir3);
    ...
    TBread<TBwaveform> readerWave = TBread<TBwaveform>(fRunNum, fMaxEvent, fMaxFile, false, "/Volumes/SSD_8TB", {8, 9, 13});
    ...
    TFile* outFile = new TFile( ( "./Waveform/Run_" + std::to_string(fRunNum) + "/" + full_ch_name + "/Run_" + std::to_string(fRunNum) + ".root" ).c_str(), "RECREATE");
    outFile->cd();
    ...
    for (int iEvt = 0; iEvt < fMaxEvent; iEvt++) {
        ...
        for (int idx = 0; idx < channel_names.size(); idx++) {
            ...
            plots.at(idx)->GetYaxis()->SetRangeUser(0, 4500);
        ...
        leg->Draw("sames");
        c->SaveAs( (TString) ( "./Waveform/Run_" + std::to_string(fRunNum) + "/" + full_ch_name + "/Evt_" + iEvt + ".png") );
    }
    outFile->Close();
}
